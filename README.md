...
| `-ct <column,[...]>`,<br>`--columntransform <column,[...]>` | Specifies the columns that should be transformed by using the column names given in the header of the file. Multiple columns can be separated by using a semicolon. | `-ct id:100->50,99->25;name:foo->co,bar->ol`<br>`--columntransform boolean:0->false;1->true` | NO | YES |
